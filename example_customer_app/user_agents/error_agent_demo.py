...
def enable_hotel_quote_crash(request: str) -> str:
...
def _arm_hotel_quote_crash(workflow_id: str | None) -> None:
    if not workflow_id:
        return
...
def _crash_during_hotel_quote_lookup(workflow_id: str | None) -> None:
    if not workflow_id:
        return
    ...
    logger.error("Runtime error: unable to get hotel quotes")
    raise RuntimeError("Runtime Error: unable to get hotel quotes.")


def _extract_hotel_quote_crash_directive(request: str) -> tuple[str, bool]:
...
def get_hotel_quotes(
    ...
    _crash_during_hotel_quote_lookup(DBOS.workflow_id)
    return _json(quotes)
...
travel_planner = Agent(
    name="travel-concierge-planner2",
    instructions=(
...
travel_coordinator = Agent(
    name="travel-concierge-coordinator2",
    instructions=(
...
@register_agent(name="travel-concierge-error-demo")
async def travel_concierge_error_demo(request: str) -> str:
    request, crash_during_hotel = _extract_hotel_quote_crash_directive(request)
    if crash_during_hotel:
        _arm_hotel_quote_crash(DBOS.workflow_id)
    ...
    for _ in range(MAX_COORDINATOR_DECISIONS):
        ...
        logger.info("travel-concierge-error-demo selected specialist: %s", next_action)
    ...
    for action in missing:
        logger.info("travel-concierge-error-demo guardrail selected missing specialist: %s", action)
        specialist_result = await agent_runner(
    ...
    logger.info("travel-concierge-error-demo final output:\n%s", final_output)
    print(f"travel-concierge-error-demo final output:\n{final_output}")
    return final_output
